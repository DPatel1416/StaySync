...
export function Logo({ inverse = false, compact = false, login = false }: { inverse?: boolean; compact?: boolean; login?: boolean }) {
  return <div className="flex items-center gap-2.5" aria-label="StaySync">
    <span data-testid="staysync-mark" className={`grid size-9 place-items-center rounded-xl shadow-sm ring-1 ring-white/15 ${login ? "bg-[#f0715d]" : "bg-[#0284c7]"}`} aria-hidden="true">
