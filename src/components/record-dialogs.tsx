...
const inputClass = "h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-900 shadow-sm focus:border-indigo-500";
const textareaClass = "w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 text-sm text-slate-900 shadow-sm focus:border-indigo-500";
...
export function ServiceRequestDialog({ defaultOpen, onCreate }: { defaultOpen?: boolean; onCreate: (draft: ServiceRequestDraft) => void }) {
  return <DialogFrame title="Create service request" description="Send an operational request to the department responsible for completing it." triggerLabel="Create service request" submitLabel="Create request" defaultOpen={defaultOpen} onSubmit={(data) => onCreate({ title: String(data.get("title")), location: String(data.get("location")), department: String(data.get("department")), priority: String(data.get("priority")), due: String(data.get("due")) })}>
    ...
    <label className="flex min-h-11 items-center gap-3 text-sm text-slate-600"><input name="notify" type="checkbox" defaultChecked className="size-4 rounded border-slate-300 text-indigo-600"/>Notify the assigned department immediately</label>
  </DialogFrame>;
}
...
export function IncidentDialog({ defaultOpen, onCreate }: { defaultOpen?: boolean; onCreate: (draft: IncidentDraft) => void }) {
  return <DialogFrame title="Report incident" description="Document what happened and route it to the department responsible for follow-up." triggerLabel="Report incident" submitLabel="Submit incident" defaultOpen={defaultOpen} icon={ShieldAlert} onSubmit={(data) => onCreate({ category: String(data.get("category")), location: String(data.get("location")), department: String(data.get("department")), priority: String(data.get("priority")), description: String(data.get("description")) })}>
    ...
    <label><Label>Supporting files</Label><input name="attachment" type="file" multiple className="block h-11 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm file:mr-3 file:border-0 file:bg-transparent file:text-sm file:font-semibold file:text-indigo-600"/></label>
  </DialogFrame>;
}

export type LostFoundDraft = { item: string; foundLocation: string; storedAt: string; followUp: string };
export function LostFoundDialog({ defaultOpen, onCreate }: { defaultOpen?: boolean; onCreate: (draft: LostFoundDraft) => void }) {
  return <DialogFrame title="Add lost and found item" description="Record where the item was found and exactly where it is being stored." triggerLabel="Add lost and found item" submitLabel="Save item" defaultOpen={defaultOpen} icon={PackageSearch} onSubmit={(data) => onCreate({ item: String(data.get("item")), foundLocation: String(data.get("foundLocation")), storedAt: String(data.get("storedAt")), followUp: String(data.get("followUp")) })}>
    ...
    <div className="grid gap-4 sm:grid-cols-2"><label><Label required>Found location</Label><input name="foundLocation" required className={inputClass} placeholder="Maple Room"/></label><label><Label required>Found date and time</Label><input name="foundAt" type="datetime-local" required className={inputClass}/></label></div>
    ...
    <label><Label>Item photo</Label><input name="photo" type="file" accept="image/*" className="block h-11 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm file:mr-3 file:border-0 file:bg-transparent file:text-sm file:font-semibold file:text-indigo-600"/></label>
