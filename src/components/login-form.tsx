...
export function LoginForm({ onModeChange }: { onModeChange?: (mode: Mode) => void } = {}) {
  ...
  return <div className={`w-full max-w-[470px] rounded-[30px] border border-[#dedfd8] bg-[#fbfcf8]/95 shadow-[0_28px_90px_rgba(21,23,19,.10),0_2px_8px_rgba(21,23,19,.04)] backdrop-blur-xl ${creatingAccount ? "p-5 sm:p-6 lg:p-5 xl:p-6" : "p-6 sm:p-8 lg:p-7 xl:p-9"}`}>
    <div className={`${creatingAccount ? "mb-3" : "mb-5"} inline-flex items-center gap-2 rounded-full border border-[#dce8bd] bg-[#f4fadf] px-3 py-1.5 text-[11px] font-bold uppercase tracking-[.13em] text-[#53631e]`}><ShieldCheck className="size-3.5"/>Secure workspace access</div>
    ...
    <div className={`${creatingAccount ? "mt-4" : "mt-6"} grid grid-cols-2 rounded-xl border border-slate-200/80 bg-slate-100/80 p-1`} role="tablist" aria-label="Account type">{(["employee", "holder"] as const).map((item) => <button key={item} type="button" role="tab" aria-selected={mode === item} onClick={() => chooseMode(item)} className={`min-h-11 rounded-lg px-3 text-sm font-semibold transition ${mode === item ? "bg-white text-slate-950 shadow-[0_1px_4px_rgba(15,23,42,.12)]" : "text-slate-500 hover:text-slate-900"}`}>{item === "employee" ? "Employee" : "General Manager"}</button>)}</div>
    <p className="mt-3 min-h-5 text-xs leading-5 text-slate-500">{mode === "employee" ? "For hotel team members using a General Manager-issued username." : "For the primary General Manager managing properties, access, and subscription settings."}</p>
    ...
    {mode === "holder" ? <p className={`${creatingAccount ? "mt-2" : "mt-5"} text-center text-sm text-slate-500`}>{creatingAccount ? "Already have an account?" : "New to StaySync?"} <button type="button" onClick={() => { setHolderAction(creatingAccount ? "signin" : "signup"); setError(""); }} className={`${creatingAccount ? "min-h-9" : "min-h-11"} px-1 font-semibold text-brand hover:text-brand-strong`}>{creatingAccount ? "Sign in" : "Create account"}</button></p> : <p className="mt-5 text-center text-xs leading-5 text-slate-400">Need access? Your hotel administrator can create or reset your employee account.</p>}
  </div>;
}
