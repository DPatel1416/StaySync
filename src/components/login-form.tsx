...
export function LoginForm({ onModeChange }: { onModeChange?: (mode: Mode) => void } = {}) {
  ...
  return <div className="w-full max-w-[470px] rounded-[30px] border border-[#dedfd8] bg-[#fbfcf8]/95 p-6 shadow-[0_28px_90px_rgba(21,23,19,.10),0_2px_8px_rgba(21,23,19,.04)] backdrop-blur-xl sm:p-8 lg:p-7 xl:p-9">
    ...
    <h1 className="text-3xl font-bold tracking-[-.045em] text-slate-950 sm:text-[2.15rem]">{creatingAccount ? "Create your hotel account" : mode === "holder" ? "Welcome back." : "Welcome to the shift."}</h1>
    <p className="mt-2.5 text-sm leading-6 text-slate-500">{creatingAccount ? "Set up your organization and first property." : mode === "holder" ? "Sign in to manage your hotels and account." : "Sign in and pick up exactly where your team left off."}</p>
