...
function DepartmentQualityScore({ department, property }: { department: string; property?: string }) { const scores = useDepartmentScores(); const current = scores.find((score) => score.department === department && (!property || score.property === property)); return current ? <CompactQualityScore department={department} score={current.score} previousScore={current.previousScore}/> : null; }
function CompactQualityScore({ department, score, previousScore }: { department: string; score: number; previousScore?: number }) { const change = previousScore === undefined ? null : score - previousScore; return <div className="flex items-center gap-3 self-start rounded-2xl border border-slate-200 bg-white px-3 py-2 shadow-soft" aria-label={`${department} quality score ${score} percent`}><div className="relative grid size-14 place-items-center rounded-full" style={{ background: `conic-gradient(#10b981 ${score * 3.6}deg, #e2e8f0 0deg)` }}><span className="grid size-[46px] place-items-center rounded-full bg-white text-sm font-bold text-slate-950">{score}%</span></div><div className="pr-1"><p className="text-xs font-semibold text-slate-800">Manager quality score</p><p className={`mt-0.5 inline-flex items-center gap-1 text-[11px] font-semibold ${change !== null && change > 0 ? "text-emerald-700" : change !== null && change < 0 ? "text-rose-700" : "text-slate-500"}`}>{change === null ? "Current department score" : <>{change > 0 ? <ArrowUp className="size-3"/> : change < 0 ? <ArrowDown className="size-3"/> : <Minus className="size-3"/>}{change > 0 ? `Up ${change}` : change < 0 ? `Down ${Math.abs(change)}` : "No change"} from previous</>}</p></div></div>; }
