...
const managerNavigationPermissions: Record<string, Permission> = {
  "Operations Log": "VIEW_REPORTS",
  "All Requests": "VIEW_SERVICE_REQUEST",
...
export function AppShell({ role, children, viewer = null }: { role: WorkspaceRole; children: React.ReactNode; viewer?: AuthenticatedViewer | null }) {
  ...
  const authorizedProperties = viewer?.properties.map((item) => item.name) ?? [];
  const [property, setProperty] = useState(authorizedProperties[0] ?? "No property assigned");
  const pathname = usePathname();
  const [user, setUser] = useState(viewer ? { name: viewer.name, title: viewer.title, isSupervisor: viewer.isSupervisor } : { name: "Authenticated user", title: `${departmentName} team member`, isSupervisor: false });
  useEffect(() => {
    if (viewer) setUser({ name: viewer.name, title: viewer.title, isSupervisor: viewer.isSupervisor });
  }, [viewer]);
  useEffect(() => {
  ...
  return <div className="department-workspace min-h-screen" data-department-theme={role}>
    ...
    <div className="lg:pl-[var(--sidebar-width)]">
      <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-t-2 border-b-slate-200/80 border-t-brand bg-white/90 px-4 backdrop-blur-md sm:px-6 lg:px-8"><div className="flex items-center gap-3"><button className="grid size-10 place-items-center rounded-lg text-slate-600 hover:bg-slate-100 lg:hidden" onClick={() => setOpen(true)} aria-label="Open navigation"><Menu className="size-5"/></button><div className="lg:hidden"><p className="text-sm font-semibold text-slate-900">{workspaceNames[role]}</p><p className="text-xs text-slate-500">{property}</p></div></div><div className="relative flex items-center gap-2"><button onClick={() => setNotificationsOpen((current) => !current)} className="relative grid size-10 place-items-center rounded-lg text-slate-500 hover:bg-slate-100" aria-label={`Notifications, ${unreadNotifications.length} unread`} aria-expanded={notificationsOpen}><Bell className="size-5"/>{unreadNotifications.length > 0 && <span className="absolute right-1 top-1 grid min-h-4 min-w-4 place-items-center rounded-full bg-rose-600 px-1 text-[10px] font-bold leading-none text-white"><span className="sr-only">Unread notifications: </span>{unreadNotifications.length}</span>}</button>{notificationsOpen && <div className="absolute right-0 top-12 z-50 w-[min(360px,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl"><div className="border-b border-slate-100 px-4 py-3"><p className="text-sm font-semibold text-slate-900">Notifications</p><p className="text-xs text-slate-500">Updates sent to {workspaceNames[role]}</p></div>{departmentNotifications.length ? <div className="max-h-80 divide-y divide-slate-100 overflow-y-auto">{departmentNotifications.map((notification) => <Link key={notification.id} href={notification.href ?? `${base}/service-requests?request=${notification.serviceRequestId}`} onClick={() => { markDepartmentNotificationRead(notification.id); setNotificationsOpen(false); }} className={`block px-4 py-3 hover:bg-slate-50 ${notification.readAt ? "bg-slate-50/50" : "bg-white"}`}><p className="text-sm font-semibold text-slate-800">{notification.title}</p><p className="mt-1 text-sm leading-5 text-slate-500">{notification.message}</p><p className="mt-1.5 text-xs text-slate-400">{notification.readAt ? "Opened" : "New"} · Sent by {notification.createdBy}</p></Link>)}</div> : <p className="px-4 py-8 text-center text-sm text-slate-500">No department notifications yet.</p>}</div>}</div></header>
