...
export default function LoginPage() {
  return <main data-login-theme="ocean" className="min-h-dvh bg-white lg:grid lg:h-dvh lg:min-h-0 lg:grid-cols-[1.08fr_.92fr] lg:overflow-hidden">
    <section className="login-grid relative hidden h-dvh overflow-hidden bg-sky-950 bg-gradient-to-br from-slate-950 via-sky-950 to-blue-700 p-10 text-white lg:flex lg:flex-col lg:justify-between xl:p-12 2xl:p-16" aria-label="About StaySync">
      <div className="absolute -right-24 top-24 size-96 rounded-full bg-cyan-300/15 blur-3xl"/><div className="absolute -bottom-40 -left-32 size-[34rem] rounded-full bg-blue-400/15 blur-3xl"/>
      <Logo inverse />
      <div className="relative z-10 max-w-xl py-4 2xl:pb-8">
        <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-3 py-1.5 text-sm text-sky-100 backdrop-blur"><Sparkles className="size-4"/>Calm operations. Exceptional stays.</div>
        <h2 className="text-5xl font-semibold leading-[1.05] tracking-[-.055em] xl:text-6xl">Hotel operations,<br/>finally in sync.</h2>
        <p className="mt-6 max-w-lg text-lg leading-8 text-sky-100/85">Coordinate teams, resolve guest needs, and keep every department aligned—without the noise.</p>
